...
class RegistrationFormWeb extends StatefulWidget {
  const RegistrationFormWeb({super.key});

  @override
  State<RegistrationFormWeb> createState() =>
      _RegistrationFormWebState();
}

class _RegistrationFormWebState extends State<RegistrationFormWeb> {
                    ...
                    onPressed: () async{
                      if(controller.countryController.text == "Bangladesh"){
                        if(controller.contactNumber.isEmpty){
                          AppUtils.errorSnackBarForWeb("Please enter your contact number");
                        }else{
                          if(formKey.currentState!.validate()){
                            controller.rememberMe ? await controller.registerAsLocalUser() : AppUtils.errorSnackBarForWeb("You must agree to Terms & Conditions");
                          }
                        }
                      } else{
                        if(formKey.currentState!.validate()){
                          controller.rememberMe ? await controller.registerAsForeignUser() : AppUtils.errorSnackBarForWeb("You must agree to Terms & Conditions");
                        }
                      }
                    },
