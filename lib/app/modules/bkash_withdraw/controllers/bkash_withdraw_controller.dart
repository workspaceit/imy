...
class BkashWithdrawController extends GetxController{

  BkashWithdrawRepo repo;
  BkashWithdrawController({required this.repo});

  TextEditingController bkashUsernameController = TextEditingController();
  ...
  final bkashForm = GlobalKey<FormState>();

  // ------------------------ this method is used to withdraw reward point from bkash -------------------------
  bool isSubmit = false;

  Future<void> withdrawRewardPointFromBkash() async{
    isSubmit = true;
    update();

    ApiResponseModel responseModel = await repo.bkashWithdrawRequest(
